...
def _update_command(self):
    ...
    self.phase = torch.clamp(self.phase + self.phase_rate * dt, 0.0, 1.0)

    # Desired references as functions of phase
    height_ref = self._height_profile(self.phase)
    pitch_ref = self._pitch_profile(self.phase)
    pitch_rate_ref = self._pitch_rate_profile(self.phase)

    self.command_tensor[:, 0] = self.phase
    self.command_tensor[:, 1] = height_ref
    self.command_tensor[:, 2] = pitch_ref
    self.command_tensor[:, 3] = pitch_rate_ref
def _height_profile(self, phase):
    # crouch → jump → land
    return torch.where(
        phase < 0.3,
        0.8 - 0.3 * phase / 0.3,           # crouch
        torch.where(
            phase < 0.6,
            0.5 + 0.6 * (phase - 0.3) / 0.3,  # jump
            1.1 - 0.3 * (phase - 0.6) / 0.4   # land
        )
    )

def _pitch_profile(self, phase):
    # 0 → -2π over flip
    return -2 * torch.pi * torch.clamp((phase - 0.25) / 0.5, 0.0, 1.0)

def _pitch_rate_profile(self, phase):
    return torch.where(
...
class BackflipCommandCfg(CommandTermCfg):
    asset_name: str
    flip_duration: float = 1.2  # seconds
